...
/*
- The Templar deploying the psi shield on player input, which would persist in a idle stance kinda like hunker.
- The attacked (miss) animation in which templar tries to absorb hit with the shield
- The attacked (hit) animation where the templar absorbs hits with his deployed shield with a few variations:
...
# Addressed
When the attack fully depletes the shield but doesn't wound the soldier, the blood effect still appears. - Fixed by using viz code in viz code. Briliant.
...
static event OnPostTemplatesCreated()
{
    ...
    AbilityTemplateManager.FindDataTemplateAllDifficulties('ParryActivate', DifficultyVariants);
...
static event OnPostTemplatesCreated()
{
     ...
     ItemMgr.FindDataTemplateAllDifficulties('ShardGauntlet_CV', DataTemplates);
